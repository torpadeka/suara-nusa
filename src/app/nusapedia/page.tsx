...
export default function NusapediaPage() {
    ...
    const [era, setEra] = useState<"sekarang" | "penjajahan" | "kerajaan">(
        "sekarang"
    );
    ...
    const [kingdomDetail, setKingdomDetail] = useState<
        DetailData[keyof DetailData] | null
    >(null);
    ...
    useEffect(() => {
        ...
        if (era === "sekarang") {
            markers = provinces.map(
                (p) => ({ ...p, type: "provinsi" } as MarkerData)
            );
            console.log("Filtered markers for provinsi:", markers);
    ...
    const handleMarkerClick = async (item: MarkerData) => {
        ...
        if (item.type === "kerajaan" && item.wilayah) {
            ...
            const matchedData =
                colonialEvents.find((e) => e.id === item.id) || null;
            setSelectedShape(null);
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        Saat ini pengguna sedang melihat informasi tentang "${
            sidebarData?.title || ""
        }".
      ...
      `;
            ...
            setChatHistory((prev) => [
                ...prev,
                { sender: "gemini", message: response },
            ]);
        } catch (err) {
            setChatHistory((prev) => [
                ...prev,
                { sender: "gemini", message: "Maaf, terjadi kesalahan." },
            ]);
    ...
    const getEraDescription = (year: number): string => {
        if (year >= 300 && year <= 699)
            return "Era Awal Kerajaan Hindu-Buddha (Abad 4–7 M)";
        if (year >= 700 && year <= 1099)
            return "Era Kejayaan Maritim & Agraris (Abad 7–11 M)";
        if (year >= 1100 && year <= 1499)
            return "Puncak Kerajaan Jawa & Transisi (Abad 12–15 M)";
        if (year >= 1500 && year <= 1699)
            return "Era Awal Kesultanan Islam (Abad 15–17 M)";
        if (year >= 1700 && year <= 1900)
            return "Masa Kesultanan Besar & Awal Kolonial (Abad 18–19 M)";
    ...
    const isColonialEvent = (
        data: Province | Kingdom | ColonialEvent | null
    ): data is ColonialEvent => {
    ...
    return (
        <div className="flex h-screen w-screen bg-gradient-to-br from-orange-900 via-orange-800 to-amber-900 font-sans">
            {/* Sidebar */}
            {/* Sidebar */}
            <aside
                    ...
                    <div className="p-6 h-full overflow-y-auto">
                        <div className="flex-1">
                            {sidebarData.status === "unsupported" && (
                                <p className="text-center text-gray-500">
                                    Info detail tidak tersedia untuk item ini.
                                </p>
                            )}
                            {sidebarData.status === "success" &&
                                sidebarData.data && (
                                    <>
                                        {sidebarData.type === "kerajaan" &&
                                            isKingdom(sidebarData.data) && (
                                                <>
                                      ...
                                      : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                                                    ...
                                                    {kingdomDetail && (
                                                        <SwiperComponent
                                                            items={
                                                                (kingdomDetail[
                                                                    activeTab as keyof typeof kingdomDetail
                                                                ] as SwiperItem[]) ||
                                                                []
                                                            }
                                                            activeTab={
                                        ...
                                        {sidebarData.type === "sekarang" &&
                                            isProvince(sidebarData.data) && (
                                                <>
                                                    <h2 className="text-2xl font-bold text-gray-800 p-3">
                                                        Provinsi{" "}
                                                        {sidebarData.title}
                                                    ...
                                                    <div className="flex border-b mb-4">
                                                        {[
                                                            {
                                                                id: "pakaian_tradisional",
                                                                label: "Pakaian Tradisional",
                                                            },
                                                            {
                                                                id: "makanan_tradisional",
                                  ...
                                  activeTab === tab.id
                                      ...
                                      : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                                                    ...
                                                    <SwiperComponent
                                                        items={
                                                            (sidebarData.data[
                                                                activeTab as keyof typeof sidebarData.data
                                                            ] as SwiperItem[]) ||
                                                            []
                                                        }
                                                        activeTab={activeTab}
                        ...
                        <div className="border-t pt-4 px-6">
                            <h3 className="text-lg font-semibold mb-2 text-gray-700">
                                Tanya tentang {sidebarData.title}
                            </h3>
                            ...
                            >
                                <input
                                    ...
                                    className="flex-1 px-3 py-2 border rounded-md text-sm w-full"
                                />
                                <button
                                    type="submit"
            ...
            <main className="flex-1 flex flex-col">
                {/* Navbar */}
                <nav className="bg-black/20 backdrop-blur-md border-b border-orange-500/20 sticky top-0 z-50">
                            ...
                            <div className="flex items-center space-x-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <select
                                        ...
                                        className="p-2 bg-black/30 border border-orange-400/30 rounded-md text-white backdrop-blur-sm cursor-pointer hover:bg-black/40 transition-colors"
                                    >
                                        <option value="sekarang">
                                            Modern Era
                                        </option>
                                        <option value="penjajahan">
                                            Colonial Era
                                        </option>
                            ...
                            <div className="pb-4 px-4">
                                <div className="flex flex-col gap-2 max-w-2xl">
                                    <div className="flex items-center gap-4">
                                        <span className="font-medium text-orange-200 min-w-fit">
                                            Year: {year} CE
                                        </span>
                                        <input
                                            type="range"
